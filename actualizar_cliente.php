...
$sql= "UPDATE cliente SET razonsocial_cliente='$razonsocial_cliente',direccion_cliente='$direccion_cliente',cuit_cliente=$cuit_cliente,email_cliente='$email_cliente',cond_iva_cliente=$cond_iva_cliente[0], estado_cliente=$estado_cliente[0] where cod_cliente=$cod_cliente";

$res = mysqli_query($con,$sql);

if($res){
    echo'<script>alert("Los datos del cliente fueron actualizados correctamente");window.location="listadoclientes.php";</script>';
}else{
     echo'<script>alert("ERROR al intentar actualizar los datos del cliente. Verifique los mismos."); history.go(-1);</script>';
}

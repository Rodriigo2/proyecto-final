  ...
  <div class="container-fluid">
    ...
    <form method="post" action="actualizar_cliente.php">
        ...
        <table>
            <tbody>
                <tr>
                    <td><label for="cod_cliente"></label>#</td>
                    <td><input type="number" readonly class="form-control" name="cod_cliente" id="cod_cliente"  value="<?php echo $reg['cod_cliente'] ?>"></td>
                </tr>
                <tr>
                    <td><label for="razonsocial_cliente">Razón Social</label></td>
                    <td><input type="text" class="form-control" name="razonsocial_cliente" id="razonsocial_cliente" placeholder="Ingrese razón social" required value="<?php echo $reg['razonsocial_cliente'] ?>"></td>
                </tr>
                <tr>
                    <td><label for="direccion_cliente">Direccíón</label></td>
                    <td><input type="text" class="form-control" name="direccion_cliente" id="direccion_cliente" placeholder="Ingrese dirección" required value="<?php echo $reg['direccion_cliente'] ?>"></td>
                </tr>

                <tr>
                    <td><label for="cuit_cliente">CUIT</label></td>
                    <td><input type="number" class="form-control" name="cuit_cliente" id="cuit_cliente" placeholder="Ingrese CUIT" required value="<?php echo $reg['cuit_cliente'] ?>"></td>
                </tr>
                <tr>
                    <td><label for="email_cliente">Correo electrónico</label></td>
                    <td><input type="email" class="form-control" name="email_cliente" id="email_cliente" placeholder="Ingrese Correo electrónico" required value="<?php echo $reg['email_cliente'] ?>"></td>
                </tr>
                <tr>
    ...
    while($regum=mysqli_fetch_array($res)){
        if($reg['cond_iva_cliente']==$regum['id_iva']){
            $sel ='selected';
        }else{
            $sel='';
        }
    ...
    ?></td></tr>
    <tr>
                    <td><label for="estado_cliente">Cond. IVA</label></td>
    ...
    if(!empty($res)){
    while($regum=mysqli_fetch_array($res)){
        if($reg['estado_cliente']==$regum['id_estado']){
            $sel ='selected';
        }else{
            $sel='';
        }
        echo '<option value="'.$regum['id_estado'].'"'.$sel.'>'.$regum['desc_estado'].'</option>';
                    }
                }
                    mysqli_close($con);
    ...
    ?></td></tr>
        ...
        <input type="submit" value="Actualizar Cliente" id="btn-cliente" name="btn-cliente" class="btn btn-success">
    </form>
    <hr>
